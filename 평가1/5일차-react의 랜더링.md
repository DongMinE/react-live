...
## 1. 데이터 바인딩
    ...
    statusMessage의 배열을 이용하여 랜덤적으로 요소를 출럭한다.

## 2. 조건부 렌더링

    vite와 react의 이미지에 3항비교식을 이용하여 텍스트값에 따른 이미지를 출력한다

## 3. 조건부 표시

    이미지의 형식이 avif, webp, png의 형식에서 브라우저가 지원하는 이미지로 출력을 한다.

## 4. 리스트 렌더링

    index를 key로 이용하여 messege를 순서대로 표시한다.
    반대로도 가능하다
